...
const EditEventCard = ({ data, handleDelete, handleEdit }) => {
  return (
    <div className={`flex flex-col  gap-x-6 items-center mb-7 pb-7  shadow-md`}>
      ...
      <div className={`w-full px-[4%]`}>
        <div className={`flex gap-x-2 items-center  mt-5`}>
          ...
          <p className="text-[11px] text-[#c2c3c7]">12:00 Noon</p>
        </div>

        <h2
          className={`sm:text-[27px] text-[#000] text-[20px] my-2 leading-[25px] sm:leading-[32px]`}
        >
          {truncateText(" Ethereum Conference.", 30)}
        </h2>
        <p className={`text-[14px] text-[#00000090]  my-4`}>
          {truncateText(
            " Networks are different Ethereum environments you can access for development, testing",
            80
          )}
        </p>
